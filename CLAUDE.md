...
将 B 站 UP 主 / 前阳光私募冠军基金经理 zettaranc（万千）的投资思维框架封装为 AI 可加载的 Skill 文件（`SKILL.md`），同时提供基于真实免费行情数据（akshare 现拉 / qcore 本机数据湖）的 Python 量化分析层（60+ 指标、30+ 战法、选股/回测/诊断）。
...
| 配置 | `cp .env.example .env`（默认 `akshare` 免 token，开箱即用；可选 `qcore`/`jnb`） |
...
| 运行测试 | `python -m pytest tests/ -v`（预期 353 passed, 1 skipped） |
...
5. **真实数据优先**：不使用 mock 数据，所有测试应基于真实数据管线（akshare/qcore 免费源；Tushare 已休眠，见 ADR-0001/0002）
