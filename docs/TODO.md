...
- [x] 新增 `pyproject.toml`（`pip install -e .` + `zt` 命令）
- [x] dotenv 统一加载（`modules/__init__.py` 一次性加载，移除 17 处重复）
- [x] 移除 try/except 兼容分支（5 个模块）
- [x] 补漏 `requirements.txt`（pandas、requests）
- [x] 修复 `SKILL.md` Windows 硬编码路径
- [x] 修复 `cli.py cmd_analyze` 运行时崩溃（`klines` 未定义）
- [x] 更新 `AGENTS.md` Python 模块规范
...
| **v2.5.0** | P0/P1 指标补全（滴滴/金叉空/出货五式/灾后重建） | ✅ 已完成 |
| **v2.5.0** | 工程化补完（pyproject.toml / dotenv 统一 / Bug 修复） | ✅ 已完成 |
| **v2.6.0** | P2 核心模块（活跃市值/三波理论/麒麟会） | ✅ 已完成 |
